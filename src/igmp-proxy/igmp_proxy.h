...
typedef struct IGMP_IFACE_NAME_LIST_T
{
	LIST_NODE 		links;	
	char 			ifaceName[IFNAMSIZ];
}IGMP_IFACE_NAME_LIST;
...
{
#ifndef MULTIPLE_UPSTREAMS
	char	upstream_ifaceName[IFNAMSIZ];
#else
	IGMP_IFACE_NAME_LIST* upstream_name_list;
	IGMP_IFACE_NAME_LIST* exclude_name_list;
#endif
...
typedef struct IGMP_ROUTER_MCB_T
{
	...
	IGMP_NETWORK_IFACE*		upstream_if_list;
	struct mfc_parent_list*		mfc_plist;
#else
	/* Cached entry to the network interface that is confgiured as UPSTREAM. */
