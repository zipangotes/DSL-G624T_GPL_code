...
)
{
...
#ifdef MULTIPLE_UPSTREAMS
	ptr_upstream_interface = (IGMP_NETWORK_IFACE *) list_get_head((LIST_NODE **)&ptr_router->upstream_if_list);
	while(ptr_upstream_interface != NULL)
	{	
		logMsg (IGMP_DEBUG, "DEBUG: Adding membership for Group: %x on upstream interface %s\n", ptr_new_group->group_address, ptr_upstream_interface->ifaceName);

		mreq.imr_ifindex	  = ptr_upstream_interface->iface_index; 
		mreq.imr_multiaddr.s_addr = ptr_new_group->group_address;
		mreq.imr_address.s_addr   = htonl(ptr_upstream_interface->ip_address.s_addr);
		if (setsockopt(ptr_upstream_interface->socket, IPPROTO_IP, IP_ADD_MEMBERSHIP, 
		...
		ptr_upstream_interface = (IGMP_NETWORK_IFACE *) list_get_next((LIST_NODE *)ptr_upstream_interface);
	}
#else
	...
	mfcctrl.mfcc_plist			= ptr_router->mfc_plist;
...
)
{
	...
	/* mfcctrl.mfc_ttls is of data type unsigned char */
#ifdef BRINDHA
	...
	if (drop_membership)
	{
		...
		while( ptr_upstream_interface != NULL)
		{
			...
			mreq.imr_address.s_addr   = htonl(ptr_upstream_interface->ip_address.s_addr);
			if (setsockopt(ptr_upstream_interface->socket, IPPROTO_IP, IP_DROP_MEMBERSHIP, 
		    	 			(void*)&mreq, sizeof(mreq)) < 0)
			{
			...
			logMsg(IGMP_DEBUG, "DEBUG: Dropping membership for Group : %x on upstream interface : %s\n", ptr_igmp_group->group_address, ptr_upstream_interface->ifaceName);
...
static void igmp_network_socket_read_data (IGMP_ROUTER_MCB*	ptr_router)
{
	...
	switch (ptr_igmp_header->type)
	{
		...
		{
			logMsg (IGMP_DEBUG,"DEBUG: Received Membership Query on %s interface.\n", ptr_network->ifaceName);
			igmp_router_received_general_query (ptr_router, ptr_network, ptr_ipheader);
			break;
		}
		case IGMP_HOST_MEMBERSHIP_REPORT:
		{
			logMsg (IGMP_DEBUG,"DEBUG: Received Version 1 Membership Report on %s interface.\n", ptr_network->ifaceName);
			igmp_router_version1_report_handler (ptr_router, ptr_network, ptr_igmp_header);
			break;
		}
		case IGMP_HOST_NEW_MEMBERSHIP_REPORT:
		{
			logMsg (IGMP_DEBUG,"DEBUG: Received Version 2 Membership Report on %s interface.\n", ptr_network->ifaceName);
			igmp_router_version2_report_handler (ptr_router, ptr_network, ptr_igmp_header);
			break;
		}
		case IGMP_HOST_LEAVE_MESSAGE:
		{
			logMsg (IGMP_DEBUG,"DEBUG: Received Leave Report on %s interface.\n", ptr_network->ifaceName);
...
int main (int argc, char *argv[])
{
...
#ifdef MULTIPLE_UPSTREAMS
	IGMP_IFACE_NAME_LIST*		ptr_upstream_ifaceName;
	IGMP_IFACE_NAME_LIST*		ptr_exclude_ifaceName;
	int 				count,check_upst_count=0,upstreams_count=-1,check_excl_iface_count=0,excl_iface_count=-1;
#else
	...
	while (index < argc)
	{
		...
		{		
			...
			if(strcasecmp (argv[index], "-c") == 0)
			{
				...
				upstreams_count = atoi(argv[index + 1]);
				index = index + 2;
			}
			if ( index >= argc)
			{
			...
			if(strcasecmp (argv[index], "-en") == 0)
			{
				...
				excl_iface_count = atoi(argv[index + 1]);
				index = index + 2;
			}
			if ( index >= argc)
			{
...
#endif
 			/* Is this the name of the upstream interface/ the list of upstream interfaces ? */
			if (strcasecmp (argv[index], "-n") == 0)
			{
#ifdef MULTIPLE_UPSTREAMS
				if(upstreams_count < 0)
				{
					/*Number of upstream interfaces is not specified in the command line*/
					print_usage ();
					return -2;
				}
				check_upst_count = index;
				for (count = 0; count < upstreams_count; count++)
				{
					check_upst_count = check_upst_count + 1;
					if (check_upst_count < argc)
					{
 						if ((strcmp(argv[check_upst_count],"-en") == NULL) && (strcmp(argv[check_upst_count],"-e") == NULL) && (strcmp(argv[check_upst_count],"-v") == NULL))
						{
							print_usage ();
							return -2;
						}
				...
				{
					index = index + 1;
					ptr_upstream_ifaceName = (IGMP_IFACE_NAME_LIST  *)my_malloc(sizeof(IGMP_IFACE_NAME_LIST));
					if( ptr_upstream_ifaceName == NULL)
					{
						logMsg(IGMP_FATAL, "IGMP_Error: unable to allocate memory for upstream interface name list\n");
						return -1;
					}
					memset((void *) ptr_upstream_ifaceName, 0, sizeof(IGMP_IFACE_NAME_LIST));
					strcpy (ptr_upstream_ifaceName->ifaceName,argv[index]);
					list_add((LIST_NODE **)&igmp_router.config.upstream_name_list,(LIST_NODE *)ptr_upstream_ifaceName);			
				}
...
#ifdef MULTIPLE_UPSTREAMS
 			/* Create a list of interfaces which should be excluded from IGMP Proxy functionality*/
			if (strcasecmp (argv[index], "-e") == 0)
			{
				if(excl_iface_count < 0)
				{
					/*Number of interfaces to be excluded is not specified in the command line*/
					print_usage ();
					return -2;
				}
				check_excl_iface_count = index;
				for (count = 0; count < excl_iface_count; count++)
				{
					check_excl_iface_count = check_excl_iface_count + 1;
					if(check_excl_iface_count < argc)
					{
						if ((strcmp(argv[check_excl_iface_count],"-c") == NULL) && (strcmp(argv[check_excl_iface_count],"-n") == NULL) && (strcmp(argv[check_excl_iface_count],"-v") == NULL)) 
						{
							print_usage ();
							return -2;
						}
				...
				{
					index = index + 1;
					ptr_exclude_ifaceName = (IGMP_IFACE_NAME_LIST  *)my_malloc(sizeof(IGMP_IFACE_NAME_LIST));
					if( ptr_exclude_ifaceName == NULL)
					{
						logMsg(IGMP_FATAL, "IGMP_Error: unable to allocate memory for upstream interface name list\n");
						return -1;
					}
					memset((void *) ptr_exclude_ifaceName, 0, sizeof(IGMP_IFACE_NAME_LIST));
					strcpy (ptr_exclude_ifaceName->ifaceName,argv[index]);
					list_add((LIST_NODE **)&igmp_router.config.exclude_name_list,(LIST_NODE *)ptr_exclude_ifaceName);
				}
			}
#else
			else
...
#ifdef MULTIPLE_UPSTREAMS
	if(igmp_router.config.upstream_name_list == NULL)
	{
		print_usage ();
		return -2;
	}
...
#ifdef MULTIPLE_UPSTREAMS
	/* The upstream interface name list has already populated while parsing the argument list above */
#else
	...
	if (igmp_router.upstream_if_list == NULL)
	{
...
static int igmp_create_network_list(IGMP_ROUTER_MCB *ptr_igmp_router)
{
	...
	IGMP_IFACE_NAME_LIST*	ptr_upstream_ifaceName;
        IGMP_IFACE_NAME_LIST*   ptr_exclude_ifaceName;
	struct mfc_parent_list*	mfc_pnode;
	IGMP_NETWORK_IFACE* 	ptr_upstream;
#endif
	...
	for (cp = buffer; cp < limit; cp = cp + sizeof (ptr_ifr->ifr_name) + sizeof(ptr_ifr->ifr_ifru))
	{
		...
		ptr_exclude_ifaceName = (IGMP_IFACE_NAME_LIST *)list_get_head((LIST_NODE **)&(&ptr_igmp_router->config)->exclude_name_list);
		while (ptr_exclude_ifaceName != NULL)
		{
			if (strcmp (ptr_exclude_ifaceName->ifaceName, ptr_network->ifaceName) == 0)
			{
				break;		
			}
			ptr_exclude_ifaceName = (IGMP_IFACE_NAME_LIST *)list_get_next((LIST_NODE *)ptr_exclude_ifaceName);
		}
		if( ptr_exclude_ifaceName != NULL)
		{
			my_free(ptr_network);
		...
		upstream = 0;		
		ptr_upstream_ifaceName = (IGMP_IFACE_NAME_LIST *)list_get_head((LIST_NODE **)&(&ptr_igmp_router->config)->upstream_name_list);
		while (ptr_upstream_ifaceName != NULL)
		{
			if (strcmp (ptr_upstream_ifaceName->ifaceName, ptr_network->ifaceName) == 0)
			{
				upstream = 1;			
			}
			ptr_upstream_ifaceName = (IGMP_IFACE_NAME_LIST *)list_get_next((LIST_NODE *)ptr_upstream_ifaceName);
		}
		if (upstream == 0)
		{
		...
		if (upstream == 1)
		{
			...
			/* Remember the entry. */
#ifdef MULTIPLE_UPSTREAMS
			/* Parent list is maintained for each cache entry */
			mfc_pnode = (struct mfc_parent_list*)my_malloc(sizeof(struct mfc_parent_list));
			if(mfc_pnode == NULL)
			{
				logMsg(IGMP_FATAL, "IGMP_Error: Unable to allocate memory for Mfc parent node \n");
				return -1;
			}
			mfc_pnode->mfcc_parent = ptr_network->iface_index;
			mfc_pnode->next = ptr_igmp_router->mfc_plist;
			ptr_igmp_router->mfc_plist = mfc_pnode;

			/* Upstream interface list */
			ptr_upstream = (IGMP_NETWORK_IFACE*)my_malloc(sizeof(IGMP_NETWORK_IFACE));
			if( ptr_upstream == NULL)
			{
				logMsg(IGMP_FATAL, "IGMP_Error: Unable to allocate memory for Mfc parent node \n");
				return -1;
			}
			memset((void *)ptr_upstream,0,sizeof(IGMP_NETWORK_IFACE));

			ptr_upstream->mode = ptr_network->mode;
			ptr_upstream->socket = ptr_network->socket;
			strcpy(ptr_upstream->ifaceName, ptr_network->ifaceName);
			ptr_upstream->iface_index = ptr_network->iface_index;
			ptr_upstream->ip_address = ptr_network->ip_address;
			ptr_upstream->flags = ptr_network->flags;
			ptr_upstream->bIsQuerier = ptr_network->bIsQuerier;
			ptr_upstream->general_query_timer = ptr_network->general_query_timer;
			ptr_upstream->igmp_version = ptr_network->igmp_version;
			ptr_upstream->group_database =ptr_network->group_database;
#else
		...
		if (upstream == 1)
		{
			ptr_upstream->vif_index = ptr_network->vif_index;
			list_add((LIST_NODE **)&ptr_igmp_router->upstream_if_list, (LIST_NODE *)ptr_upstream);
		}
#endif
	...
	/* BUG: On no downstream interface detected report error message. */
	if (downstream == 0)
		return -1;
...
#ifdef MULTIPLE_UPSTREAMS
	/* Clean and restore the upstream interface list */
	ptr_upstream_interface = (IGMP_NETWORK_IFACE *) list_remove((LIST_NODE **)&ptr_router->upstream_if_list);
	while( ptr_upstream_interface != NULL)
	{
		logMsg(IGMP_DEBUG, "DEBUG: Cleaning upstream interface %s from upstream interface List\n",ptr_upstream_interface->ifaceName);
		igmp_proxy_clean_interface (ptr_router, ptr_upstream_interface);
		my_free (ptr_upstream_interface);
		ptr_upstream_interface = (IGMP_NETWORK_IFACE *) list_remove ((LIST_NODE **)&ptr_router->upstream_if_list);
	}
	/* Clean and restore the upstream name list */
	ptr_upstream_ifaceName = (IGMP_IFACE_NAME_LIST  *) list_remove ((LIST_NODE **)&ptr_router->config.upstream_name_list);
	while( ptr_upstream_ifaceName != NULL)
	{
		logMsg(IGMP_DEBUG, "DEBUG: Cleaning upstream interface %s from Upstream Name list\n", ptr_upstream_ifaceName->ifaceName);
		my_free ((void *)ptr_upstream_ifaceName);
		ptr_upstream_ifaceName = (IGMP_IFACE_NAME_LIST *) list_remove ((LIST_NODE **)&ptr_router->config.upstream_name_list);
	}

	/* Clean and restore the exclude interface name list */
	ptr_exclude_ifaceName = (IGMP_IFACE_NAME_LIST  *) list_remove ((LIST_NODE **)&ptr_router->config.exclude_name_list);
	while( ptr_exclude_ifaceName != NULL)
	{
		logMsg(IGMP_DEBUG, "DEBUG: Cleaning interface %s from  Exclude interface Name list\n", ptr_exclude_ifaceName->ifaceName);
		my_free ((void *)ptr_exclude_ifaceName);
		ptr_exclude_ifaceName = (IGMP_IFACE_NAME_LIST *) list_remove ((LIST_NODE **)&ptr_router->config.exclude_name_list);
	}

	/* Clean and restore the upstream mfc list*/
	while(ptr_router->mfc_plist != NULL)
	{
		logMsg(IGMP_DEBUG, "DEBUG: Cleaning parent list\n");
		mfc_pnode = ptr_router->mfc_plist;
		ptr_router->mfc_plist = ptr_router->mfc_plist->next;
		my_free ((void *)mfc_pnode); 		
	}	
#else
...
{
#ifdef MULTIPLE_UPSTREAMS
	printf ("Usage: IGMPProxy [-v] -c X -n <iface1>..<ifaceX> [-en Y -e <iface1>..
         				..<ifaceY>.\n");
	printf ("-c X 			: X is the no. of IGMP Proxy 
					  enabled upstream interfaces\n");
	printf ("-n <iface1>.. <ifaceX> :
					  Names of IGMP Proxy enabled interfaces\n");
	
	printf ("-en Y 			: Y is the no. of interfaces excluded from 
					  IGMP Proxy functionality\n");
	printf ("-e <iface1>.. <ifaceY> :
					  Names of interfaces to be excluded from 
					  IGMP Proxy functionality\n");
	printf ("-v             	: Verbose mode - Print debug messages\n");

	printf ("Example: IGMPProxy -v -c 2 -n nas0 nas1 -en 1 -e nas2\n\n");
#else
	printf ("Usage: IGMPProxy [-v] -n <interface name>.\n");
	printf ("-n <interface name>  : Name of the interface on which the router is present\n");
	printf ("-v                   : Verbose mode - Print debug messages\n");
	printf ("Example: IGMPProxy -v -n nas0\n\n");
#endif
	return;
}
