...
#else	
    fprintf(stderr, "               [ offset OFFSET_SPEC ]\n");
#endif	
...
static int pack_key(struct tc_u32_sel *sel, __u32 key, __u32 mask, int off, int offmask, int range_flag)
{
	int i;
	int hwm = sel->nkeys;
...
static int pack_key32(struct tc_u32_sel *sel, __u32 key, __u32 mask, int off, int offmask)
{
	key = htonl(key);
	mask = htonl(mask);
	return pack_key(sel, key, mask, off, offmask, 0);
}
...
static int parse_ip_addr(int *argc_p, char ***argv_p, struct tc_u32_sel *sel, int off)
{
	...
	if (pack_key(sel, addr.data[0], mask, off, offmask, 0) < 0)
		return -1;
	res = 0;
...
static int parse_ip6_addr(int *argc_p, char ***argv_p, struct tc_u32_sel *sel, int off)
{
	...
	for (i=0; i<plen; i+=32) {
		if (((i+31)&~0x1F)<=plen) {
			if ((res = pack_key(sel, addr.data[i/32], 0xFFFFFFFF, off+4*(i/32), offmask, 0)) < 0)
				return -1;
		} else if (i<plen) {
			__u32 mask = htonl(0xFFFFFFFF<<(32-(plen-i)));
			if ((res = pack_key(sel, addr.data[i/32], mask, off+4*(i/32), offmask, 0)) < 0)
				return -1;
		}
	}
	res = 0;
