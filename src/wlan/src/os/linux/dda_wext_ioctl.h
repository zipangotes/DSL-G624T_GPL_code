...
/*!
 *                                                                         
 *   \file  dda_wext_ioctl.h                                                
 *   \brief PURPOSE: headers and definitions for public ioctl interface 													       
 *                                                                         
 */
...
/* Demiurg: WME */
#define WCFG_NUM_OF_QOS_CONFIGS 2
...
#define TIAP_PARAM_NUM_MULTIPLE_SSID                    (5) 
#define TIAP_PARAM_MAX_PRIMARY_SSIDS					(1)
#define TIAP_PARAM_MAX_SECONDARY_SSIDS					(3)
#define TIAP_PARAM_MAX_INTERNAL_SSIDS					(1)
...
typedef enum {
  ...
  TIAP_PARAM_RD_ISRAEL  = 0x43,
  TIAP_PARAM_RD_US      = 0x50,
  TIAP_PARAM_RD_WORLD   = 0x51,
  TIAP_PARAM_RD_CUSTOM  = 0x60,
...
enum {
	...
	TIAP_PARAM_RADAR_DETECT_MODE,	/* 25 */
	TIAP_PARAM_AP_BRIDGE_MODE,
    TIAP_PARAM_WDS_ENABLE,
	TIAP_PARAM_EEPROM_LESS_ENABLE,
	/*complex*/
	...
	TIAP_PARAM_QOS,
	TIAP_PARAM_BIT,
	TIAP_PARAM_HW_INTERF_CFG,
	TIAP_PARAM_EEPROM,
	TIAP_PARAM_DIE_ID,
};

#define TIAP_WDS_SSID  "\x1BTI_WDS" /*include non-printable character ESC*/
...
typedef enum { 
	...
	TIAP_PARAM_WD_INTERF,  /* The whole ACI/NB interference WA in driver */
...
#define TIAP_PARAM_EEPROM_MAX_SIZE 940

typedef struct {
	char image[TIAP_PARAM_EEPROM_MAX_SIZE];
	__u32 len;
	tiap_param_die_id_t die_id;
}tiap_param_eeprom_t;
...
typedef enum
{
	TIAP_PARAM_SSID_NON_ACTIVE,
	TIAP_PARAM_SSID_PRIMARY,
	TIAP_PARAM_SSID_SECONDARY,
	TIAP_PARAM_SSID_INTERNAL
}tiap_param_ssid_type_t;

typedef struct
{
	__s8 ssid[IW_ESSID_MAX_SIZE+1];
	__u8 sec_profile_index; /* index of corresponding security profile */
	__u16 vlan;
	tiap_param_ssid_type_t ssid_type;
}tiap_param_multi_ssid_elem_t;

typedef struct {
	__u16 secondary_enabled;
	tiap_param_multi_ssid_elem_t tbl[TIAP_PARAM_NUM_MULTIPLE_SSID];
...
typedef struct {
	__u8 cwMin, aifsn, shortRetryLimit, longRetryLimit, ackPolicy;
	__u16 cwMax, txopLimit11b, txopLimit11g, txOpContinuation;
	__u32 txMaxLifetime;
} tiap_param_qos_ac_t;

/* Duplicates definitions in QoS/QoSACParams.h */
#define TIAP_PARAM_QOS_MAX_CP 64
/*
typedef struct {
	__u8 codePoint, vlanPriority;
} tiap_param_qos_diffserv_map_t;
*/
typedef struct {
	__u8 numOfCP;
	__u8 map[TIAP_PARAM_QOS_MAX_CP];
} tiap_param_qos_diffserv_t;

typedef struct {
	__u8 enabled;
	__u8 classifierMode;
	//	wdrv_QoSClassifierMode_t mode;
	tiap_param_qos_ac_t edcf_info[2][4];
	tiap_param_qos_diffserv_t diffserv;
} tiap_param_qos_t;

typedef enum {
	TIAP_PARAM_QOS_CLASSIFIER_MODE_VLAN = 0,
	TIAP_PARAM_QOS_CLASSIFIER_MODE_DIFFSERV,
	TIAP_PARAM_QOS_CLASSIFIER_MODE_UDP_PORT,
	TIAP_PARAM_QOS_CLASSIFIER_MODE_UNDEFINED
} tiap_param_qos_classifier_mode_t;

enum {
	TIAP_PARAM_QOS_AC_AP_PARAMS = 0,
	TIAP_PARAM_QOS_AC_STA_PARAMS = 1
}; 

enum {
	TIAP_PARAM_QOS_CLASSIFIER_AC_BE = 0,
	TIAP_PARAM_QOS_CLASSIFIER_AC_BK = 1,
	TIAP_PARAM_QOS_CLASSIFIER_AC_VI = 2,
	TIAP_PARAM_QOS_CLASSIFIER_AC_VO = 3,
	TIAP_PARAM_QOS_CLASSIFIER_BROADCAST = 4,
	TIAP_PARAM_QOS_CLASSIFIER_AC_UNDEFINED = -1
};

enum {
	TIAP_PARAM_QOS_ACK_POLICY_ACK_IMMEDIATE = 0,
	TIAP_PARAM_QOS_ACK_POLICY_NO_ACK = 1
};
...
typedef union
{
	...
	__u8 brcst_default_rate;
	__u8 mcst_default_rate;
	__u8 tx_default_rate;
	__u8 ch;
	__u8 band;
	__u8 pbcc_mode;
	__u8 rate_adapt;
...
typedef enum {
	...
	TIAP_GENERIC_IOCTL_DOWNLOAD_FW,
	TIAP_GENERIC_IOCTL_SET_BURST_LIMIT,
	TIAP_GENERIC_IOCTL_GET_BURST_LIMIT,
    TIAP_GENERIC_IOCTL_SEND_WDS_PKT,
    TIAP_GENERIC_IOCTL_SET_WDS_PID,
    TIAP_GENERIC_IOCTL_WDS_LINK_ADD,
    TIAP_GENERIC_IOCTL_WDS_LINK_REMOVE,
    TIAP_GENERIC_IOCTL_WDS_STAT_GET,
    TIAP_GENERIC_IOCTL_WDS_IFC_STAT_GET,
    TIAP_GENERIC_IOCTL_WDS_DRV_STAT_GET,
    TIAP_GENERIC_IOCTL_WDS_BRG_STAT_GET,
    TIAP_GENERIC_IOCTL_BSS_SERVICE_STATE,
    TIAP_GENERIC_IOCTL_AFTER_GO_FLAG_GET,
...
/* Structs to pass BIT commands arguments to Driver through IOCTLs */
typedef enum {
    BIT_FCC,
    BIT_TELEC
} bitCmdType_e;
...
typedef struct {
    bitCmdType_e    cmdType;
	__u32           band;
	__u32           channel;
	__u32           rate;
	__s32           mode;
} tiap_param_bit_args_t;
/* End of BIT commands structs */

/* HW interference handling configuration parameters */
typedef enum {

	TIAP_PARAM_INPUT_CCA,
	TIAP_PARAM_TX_CCA,
	TIAP_PARAM_QUALIF_CCA,
	TIAP_PARAM_STOMP_RX,
	TIAP_PARAM_STOMP_TX,
}tiap_param_hw_interf_cmd_t;

/* HW interference handling parameter type*/
typedef struct {
	tiap_param_hw_interf_cmd_t cmd;
	__u8	value;
}tiap_param_hw_interf_t;
...
union the_largest_struct {
	...
	/* Demiurg: WME */
	tiap_param_qos_t qos;
    tiap_param_bit_args_t bit;
	tiap_param_hw_interf_t hw_interf;
	tiap_param_eeprom_t eeprom;
	tiap_param_die_id_t die_id;
};
...
struct configured_params{
	...
	/*complex data types*/
	__u8				auth_tbl;
	__u8				wd_en, wd_interf, wd_rxfree, wd_beacons, wd_txstuck, wd_fcserrs, wd_aci, wd_nb;
	__u8 				hw_input_cca, hw_tx_cca, hw_qualif_cca, hw_stomp_rx, hw_stomp_tx;
	__u8				access_list;
	...
	__u8				oper_rate[TIAP_PARAM_WONE_DUAL_BAND], oper_add[TIAP_PARAM_WONE_DUAL_BAND],
					oper_ofdm[TIAP_PARAM_WONE_DUAL_BAND], brcst_rate[TIAP_PARAM_WONE_DUAL_BAND], ch[TIAP_PARAM_WONE_DUAL_BAND],
					mcst_rate[TIAP_PARAM_WONE_DUAL_BAND],tx_rate[TIAP_PARAM_WONE_DUAL_BAND],
