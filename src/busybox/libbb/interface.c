 ...
 *
 * Version:     $Id: interface.c,v 1.1.1.1 2003/11/25 22:13:57 jharrell Exp $
 *
...
#ifndef SIOCSIFASYMMTU
#define SIOCGIFASYMMTU	0x89A0
#define SIOCSIFASYMMTU	0x89A1
#endif
...
struct interface {
    ...
    int mtu;			/* MTU value             */
    int asymmtu;		/* Asymmetric MTU value  */
...
static int if_fetch(struct interface *ife)
{
	...
	ife->mtu = ifr.ifr_mtu;
	if (ioctl(skfd, SIOCGIFASYMMTU, &ifr) < 0)
	ife->asymmtu = 0;
    else
	ife->asymmtu = ifr.ifr_mtu;
...
#endif
	printf(_("  ASYMMTU:%d"), ptr->asymmtu);
    printf("\n");
