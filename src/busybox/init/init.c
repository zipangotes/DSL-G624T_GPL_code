...
#ifndef DEBUG_INIT
static void shutdown_system(int sig)
{
	sigset_t block_signals;

	/* Hook up NSP to SIGUSR1 */
	if(sig==15)
	{
		...
		kill(-1, SIGTERM);sleep(1);sync();kill(-1, SIGKILL);sleep(1);

		execve("/var/upgrader", 0, 0);
		message(CONSOLE|LOG, "\n\rFailed to start the NSP upgrader.\n");
	}
...
extern int init_main(int argc, char **argv)
{
	...
	while (1) {
		...
		for (a = init_action_list; a; a = a->next) {
			if(!a->pid)
				continue;
			wpid = waitpid(a->pid, 0, WNOHANG);
			if (wpid == a->pid || (wpid < 0 && errno == ECHILD)) {
