...
//			UpnpAddToPropertySet(&PropSet, "PhysicalLinkStatus", m_ipcon->IPCon_GetIfStatStr());
			...
			GateGetPhysicalLinkStatus(LinkStatus);
			UpnpAddToPropertySet(&PropSet, "PhysicalLinkStatus", LinkStatus);
...
int GateGetPhysicalLinkStatus(char *LinkStatus)
{
   FILE *fd;
   int retval;
   char buffer[100];

   fd=fopen("/proc/avalanche/avsar_modem_training", "r");
   if(!fd)
   {
       printf("Error: Unable to open DSL proc entry\n");
       return -1;
   }
   retval = read (fileno(fd), (void *)buffer, sizeof (buffer));
   if (retval < 0)
   {
   	printf("Error: Unable to read the DSL proc entry");
      fclose(fd);
      return -1;
   }
   fclose(fd);
	/* retval is modulation type, Linkstatus is status of the link */
   sscanf (buffer, "%s %d", LinkStatus, &retval);
	if (strcmp (LinkStatus, "SHOWTIME") == 0)
		...
		struct Upnp_Action_Request *ca_event)
{
   ...
   char *pos;
	char buff[500];
	unsigned int UpstreamMaxBitRate = 0, DownstreamMaxBitRate = 0;
	char LinkStatus[15];
   ...
   pos=strstr(pbuff, "US Connection Rate:");
   pos += strlen("US Connection Rate:");
   if(pos)
	{
		retval = sscanf(pos," %u", &UpstreamMaxBitRate);
      printf("retval = %d, upstream = %u", retval, UpstreamMaxBitRate);
   }
   pos=strstr(pbuff, "DS Connection Rate:");
   pos += strlen("DS Connection Rate:");
   if(pos)
	{
		retval = sscanf(pos," %u", &DownstreamMaxBitRate );
      printf("retval = %d, downstream = %u", retval, DownstreamMaxBitRate );
   }
   free(pbuff);

	retval = GateGetPhysicalLinkStatus(LinkStatus);
   if (retval < 0)
   {
   	ca_event->ErrCode = UPNP_SOAP_E_ACTION_FAILED;
      strcpy(ca_event->ErrStr, "Action Failed");
   ...
   sprintf (buff, "<NewWANAccessType>DSL</NewWANAccessType><NewLayer1UpstreamMaxBitRate>%u</NewLayer1UpstreamMaxBitRate><NewLayer1DownstreamMaxBitRate>%u</NewLayer1DownstreamMaxBitRate><NewPhysicalLinkStatus>%s</NewPhysicalLinkStatus>", UpstreamMaxBitRate*1000, DownstreamMaxBitRate*1000, LinkStatus);
...
//				result=m_list.PortMapAdd(NULL, proto, address, atoi(ext_port), int_ip, atoi(int_port), 1, desc, 0);
				result=PortMapAdd(&(gate->m_list), NULL, proto, address, atoi(ext_port), int_ip, atoi(int_port), 1, desc, 0);
				if (result==1)
				{
					int size=0;
					for (itr = gate->m_list.m_pmap; itr; itr=itr->next) {
