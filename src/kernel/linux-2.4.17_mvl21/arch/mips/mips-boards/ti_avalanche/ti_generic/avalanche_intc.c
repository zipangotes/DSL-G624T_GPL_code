...
typedef int  (*AV_VLYNQ_REQ_IRQ_FN)(unsigned int irq, void (*handler)(int, void *, struct pt_regs *),
...
static AV_VLYNQ_DISABLE_IRQ_FN  p_av_vlynq_disable_irq_fn  = NULL;
static AV_VLYNQ_EN_IRQ_FN       p_av_vlynq_enable_irq_fn   = NULL;
...
static AV_VLYNQ_IRQ_LIST_FN     p_av_vlynq_irq_list_fn     = NULL;
...
void disable_irq(unsigned int irq_nr)
{
	...
	{
            if(p_av_vlynq_disable_irq_fn)
                p_av_vlynq_disable_irq_fn(irq_nr);
...
void enable_irq(unsigned int irq_nr)
{
	 ...
	 */
        if(irq_nr >= AVALANCHE_INTC_END)
	{
            if(p_av_vlynq_enable_irq_fn)
                p_av_vlynq_enable_irq_fn(irq_nr);
...
int get_irq_list(char *buf)
{
	...
	for (i = 0; i < MIPS_EXCEPTION_OFFSET; i++, num++)
	{
        ...
        if(p_av_vlynq_irq_list_fn)
            len += p_av_vlynq_irq_list_fn(buf + len);
		...
		unsigned long irqflags, const char * devname, void *dev_id)
{
        ...
        if(irq >= AVALANCHE_INTC_END)
	{
            if(p_av_vlynq_req_irq_fn)
                p_av_vlynq_req_irq_fn(irq, handler,irqflags,
                                      devname, dev_id);
...
void init_IRQ(void)
{
...
#ifdef CONFIG_AVALANCHE_INTC_PACING
	avalanche_hw0_ipaceregs->ipacep = (avalanche_get_intc_input_freq()/1000000)*4;
#endif
	...
	action->next = 0;
        ...
        p_av_vlynq_disable_irq_fn  = avalanche_vlynq_disable_irq;
        p_av_vlynq_enable_irq_fn   = avalanche_vlynq_enable_irq;
        p_av_vlynq_set_irq_pol_fn  = avalanche_vlynq_set_irq_polarity;
        p_av_vlynq_get_irq_pol_fn  = avalanche_vlynq_get_irq_polarity;
        p_av_vlynq_set_irq_type_fn = avalanche_vlynq_set_irq_type;
        p_av_vlynq_get_irq_type_fn = avalanche_vlynq_get_irq_type;
        p_av_vlynq_req_irq_fn      = avalanche_vlynq_request_irq;
        p_av_vlynq_free_irq_fn     = avalanche_vlynq_free_irq;
        p_av_vlynq_irq_list_fn     = avalanche_vlynq_irq_list;    
