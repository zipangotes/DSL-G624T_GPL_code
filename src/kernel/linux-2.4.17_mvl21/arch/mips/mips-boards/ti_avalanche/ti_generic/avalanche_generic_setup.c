...
#define BasePSP_NAME    "DSL-PSP"
#define BasePSP_VERSION "4.6.1.9 on BasePSP Version 7.1.0.8"
#else
#define BasePSP_NAME    "BasePSP"
#define BasePSP_VERSION "7.1.0.8"
#endif
                                             ...
                                             int count, int *eof, void *data)
{
    ...
    if(len<=limit)
        len+= sprintf(buf+len, "\nLinux OS %s version %s %s %s Avalanche SOC Version: 0x%x operating in %s mode Cpu Frequency:%u MHZ System Bus frequency: %u MHZ\n\n", BasePSP_NAME, BasePSP_VERSION, __DATE__, __TIME__, PAL_sysGetChipVersionInfo(), cache_mode[cache_index], cpu_freq/1000000, 2*avalanche_get_vbus_freq()/1000000);
                                     ...
                                     int *eof, void *data)
{
    ...
    for ( i = last_index; ; i++, last_index++ )
    {
        ...
        if( (pvar=adam2_env_get_variable(i))!=NULL ) 
        {
            ...
            {
                /* Calculate next line length, store in tmplen */
                tmplen = strlen( pval ) + strlen( pvar ) + 2;
                
                /* Check if there is enough space in the buffer */
                if ( len + tmplen > limit )
                {
                    /* We exceeded data length. */
                    break_flag = 1;
    ...
    /* Mark EOF if we broke the iteration, or buffer length is smaller than
...
{
  if(0x000b != (*((volatile unsigned int*) AVALANCHE_CVR) & 0xffff))
      return (1); /* The known SoC which does not have CPMAC on VBUS is Apex. */
  else
      return (0);
} 
