...
#if defined(CONFIG_MIPS_V2USB_EVM)

/* Code for V2U intiialization */
typedef struct {
	        UINT32 int_vector;
		UINT32 map_vector;
                VLYNQ_DEV_TYPE dev;
...
/*
  VLYNQ0 - [0] usbs_usb20_pls
           ...
           [5] vlynq1_int_pls
           [6] gpio_intpls_o
           [7] ext_wakeup_event_intlvl_o */
VECMAP vecmap[] = {  
	             {0,2,VLYNQ_REMOTE_DVC,VLYNQ_INTR_ACTIVE_HIGH,VLYNQ_INTR_PULSED,1},
	             {1,3,VLYNQ_REMOTE_DVC,VLYNQ_INTR_ACTIVE_HIGH,VLYNQ_INTR_PULSED,1},
	             {2,4,VLYNQ_REMOTE_DVC,VLYNQ_INTR_ACTIVE_LOW,VLYNQ_INTR_LEVEL,1},
	             {3,5,VLYNQ_REMOTE_DVC,VLYNQ_INTR_ACTIVE_LOW,VLYNQ_INTR_LEVEL,0},/*check from here*/
		     {4,6,VLYNQ_REMOTE_DVC,VLYNQ_INTR_ACTIVE_HIGH,VLYNQ_INTR_LEVEL,0},
		     {5,7,VLYNQ_REMOTE_DVC,VLYNQ_INTR_ACTIVE_HIGH,VLYNQ_INTR_PULSED,0},
		     {6,8,VLYNQ_REMOTE_DVC,VLYNQ_INTR_ACTIVE_HIGH,VLYNQ_INTR_PULSED,0},
		     {7,9,VLYNQ_REMOTE_DVC,VLYNQ_INTR_ACTIVE_HIGH,VLYNQ_INTR_PULSED,0}
                  };
	             
#endif /* CONFIG_MIPS_V2USB_EVM */

#ifdef CONFIG_MIPS_V2USB_EVM
void  vlynq_dev_init()
{
		int ret=0;
    ...
    *reset_base |= (1 << AVALANCHE_LOW_VLYNQ_RESET_BIT) | (1 << AVALANCHE_HIGH_VLYNQ_RESET_BIT); 
    vlynq_delay(1000000); /* Allowing sufficient time to VLYNQ to settle down.*/


	/* configure for v2u,titan sdb VLYNQ0 interface */
	vlynqDevice0.dev_idx = 0;
	vlynqDevice0.module_base = 0xa8611c00;

	prom_printf("Configuring Vlynq0 for V2USB.\n");
		vlynqDevice0.clk_source = VLYNQ_CLK_SOURCE_LOCAL;
		   vlynqDevice0.clk_div    =0x02;

                       
		   vlynqDevice0.local_mem.Txmap =0x0c000000;  
		    vlynqDevice0.local_mem.RxOffset[0] = 0x14000000;
			vlynqDevice0.local_mem.RxOffset[1] = 0;
			vlynqDevice0.local_mem.RxOffset[2] = 0;
			vlynqDevice0.local_mem.RxOffset[3] = 0;
			vlynqDevice0.local_mem.RxSize[0] = 0x04000000;
			vlynqDevice0.local_mem.RxSize[1] = 0;
			vlynqDevice0.local_mem.RxSize[2] = 0;
			vlynqDevice0.local_mem.RxSize[3] = 0;
				   
			vlynqDevice0.remote_mem.Txmap =	0x00000000;
            vlynqDevice0.remote_mem.RxOffset[0] = 0x0c000000;
			vlynqDevice0.remote_mem.RxOffset[1] =	0;
			vlynqDevice0.remote_mem.RxOffset[2] = 0;
			vlynqDevice0.remote_mem.RxOffset[3] = 0;
			vlynqDevice0.remote_mem.RxSize[0]   = 0x00030000;
			vlynqDevice0.remote_mem.RxSize[1]   =0;
			vlynqDevice0.remote_mem.RxSize[2]   =0;
			vlynqDevice0.remote_mem.RxSize[3]   =0;
        
            vlynqDevice0.local_irq.intLocal =VLYNQ_INT_LOCAL;
			vlynqDevice0.local_irq.intRemote =VLYNQ_INT_ROOT_ISR;
            vlynqDevice0.local_irq.map_vector =0;
			vlynqDevice0.local_irq.intr_ptr = 0;
	
			vlynqDevice0.remote_irq.intLocal =VLYNQ_INT_REMOTE;
			vlynqDevice0.remote_irq.intRemote =VLYNQ_INT_ROOT_ISR;
		    vlynqDevice0.remote_irq.map_vector = 1;
			vlynqDevice0.remote_irq.intr_ptr = 0;

    ret = vlynq_init(&vlynqDevice0,VLYNQ_INIT_PERFORM_ALL/*&(~VLYNQ_INIT_CHECK_LINK)*/);
			
    if(ret==0)
    { int cnt, i;

			/* Vlynq init successful */
        vlynq_init_status[0] = 1;
		
      /* Establish Interrupt line to bit position mappings */
       cnt = sizeof(vecmap) / sizeof(VECMAP) ; /*number of array entries */
       for(i=0;i<cnt;i++)
        {
		vlynq_interrupt_vector_set(&vlynqDevice0,
			           vecmap[i].int_vector,
					   vecmap[i].map_vector,
                       vecmap[i].dev,
					   vecmap[i].pol,
               ...
               if (vecmap[i].int_enable)
		       vlynq_interrupt_enable(&vlynqDevice0,vecmap[i].dev,vecmap[i].map_vector);
	       else
		       vlynq_interrupt_disable(&vlynqDevice0,vecmap[i].dev,vecmap[i].map_vector);
        }
       
       
    }
    else
        prom_printf("VLYNQ INIT FAILED: Please try cold reboot.Ret value%d \n",ret);

	/* for complete Board setup initialize Vlynq1 device structure  also */
		   vlynqDevice1.dev_idx=1;
	       vlynqDevice1.module_base=0xa8611300;
...
#endif /* CONFIG_MIPS_V2USB_EVM */
#endif /* CONFIG_MIPS_AVALANCHE_VLYNQ */
