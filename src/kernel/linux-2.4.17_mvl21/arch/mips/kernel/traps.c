...
#endif /* CONFIG_MIPS_AVALANCHE _SOC */
	}
...
void __init trap_init(void)
{
	...
	memcpy((void *)(KSEG0 + 0x0),   &jump_tlb_miss, 0x80);            /* TLB miss (R4Kc core) */
	memcpy((void *)(KSEG0 + 0x80),  &jump_tlb_miss_unused, 0x80);     /* TLB miss (unused by R4Kc core */
	memcpy((void *)(KSEG0 + 0x100), &jump_cache_error, 0x80);         /* Cache error exception */
	memcpy((void *)(KSEG0 + 0x180), &jump_general_exception, 0x80);   /* General excpetion (handler called) */
	memcpy((void *)(KSEG0 + 0x200), &jump_dedicated_interrupt, 0x80); /* Dedicated interrupt (mipsIRQ) */
...
#if defined(CONFIG_MIPS_AVALANCHE_SOC)
			memcpy((void *)(AVALANCHE_VECS_KSEG0 + 0x180), &except_vec3_generic,
			       0x80);
#else
