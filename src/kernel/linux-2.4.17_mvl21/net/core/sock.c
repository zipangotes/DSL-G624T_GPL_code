 ...
 *
 *
 * Version:	$Id: sock.c,v 1.1 2002/04/09 18:01:44 jharrell Exp $
 *
				     ...
				     unsigned long data_len, int noblock, int *errcode)
{
	...
	while (1) {
		...
		if (atomic_read(&sk->wmem_alloc) < sk->sndbuf) 
        {
            /* PANKAJ: This code has been added to allocate memory of the
             * static memory pool instead of the dynamic allocater. The Static
             * memory manager ensures enough headroom for PPP and 2684
             * protocols so that there is no copy involved at the lower
             * layers. Static Allocation works only for a MAX of 1500 bytes.
             * Larger packet sizes still use the Linux Orignal kernel
             * allocation. */
#ifdef TI_SLOW_PATH
			skb = alloc_skb(header_len, sk->allocation);
#else
            if (header_len < 1500)
                skb = ti_alloc_skb(header_len, sk->allocation);
            else
                skb = alloc_skb(header_len, sk->allocation);
#endif
			if (skb) {
