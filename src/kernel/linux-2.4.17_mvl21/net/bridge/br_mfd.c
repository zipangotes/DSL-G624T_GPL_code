...
#define FLOOD_MCAST(x)	(((x) & htonl(0xFFFF0000)) == htonl(0xE0000000))
...
__u8 br_handle_mc(struct net_bridge *br,struct sk_buff *skb,int mode)
{
	...
	if (ntohs(ethh.h_proto) == ETH_P_IP)
	{
		...
		/* Handling less frequent other multicast address destined traffic here, 
		 * after handling more frequent high priority multicast data
		 */	
			if ((mode == FORWARD) && (FLOOD_MCAST(iph.daddr))) {
						DEBUGP("At Bridge flooding %x MCAST packets\n", iph.daddr);
				
						//printk("At Bridge flooding %x MCAST packets\n", iph.daddr);
#ifndef TI_SLOW_PATH
						br_flood(br,skb,0);
#else
						br_flood_deliver(br, skb, 0);
