...
static void wb_netif_rx (struct sk_buff *skb, WAN_BRIDGE_DEVICE* ptr_wan_device)
{
    ...
    {
        /* Device is not UP. Drop the packet */
   	    ptr_wan_device->ptr_wan_bridge->rx_packet_dropped++;
        wb_kfree_skb (skb);
    }
    return;
}
...
int wb_receive_frame (struct sk_buff *skb, void* ptr_handle)
{
    ...
    switch (ptr_wan_bridge->wb_mode)
    {
        case WAN_BRIDGE_VLAN_MODE:
        {
            ...
            /* Jyothi Get the p bit from VLAN header and store in the skb->priority.
             * This will be used by priowrr_classify to identify the queue */

            if (skb->dev->if_port == AVALANCHE_WLAN_PORT_ID)
            {
                char    priority;    
                priority = (vlan_TCI & 0xE000) >> 13; 
                wblogMsg (WB_LOG_DEBUG, "DEBUG: Received Packet with VLAN: %d prio: %d.\n",vlan_id, priority,0,0,0);
                switch (priority) 
                {
                    case 7 :
                    case 6 : skb->priority = 0xFFFF;
                             break;
                    case 5 :
                    case 4 : skb->priority = 0xFAAD;
                             break;
                    case 3 :
                    case 2 : skb->priority = 0xFADD;
                             break;
                    default: skb->priority = 0xFDDD;
                             break;
                }
                wblogMsg (WB_LOG_DEBUG, "DEBUG: SKB Prio: %x.\n",skb->priority,0,0,0,0);
            }
...
/* Jyothi Check if pbits are correctly set */ 
