 ...
 *
 *	$Id: br_if.c,v 1.4 2004/12/16 14:12:06 gopal_kr Exp $
 *
...
struct net_bridge **__find_br(char *name)
{
	struct net_bridge **b;
...
static struct net_bridge *new_nb(char *name)
{
	...
	br->root_path_cost = 0;
	br->root_port = 0;
	br->bridge_max_age = br->max_age = 6 * HZ;
	br->bridge_hello_time = br->hello_time = 2 * HZ;
	br->bridge_forward_delay = br->forward_delay = 4 * HZ;
	...
	/*Set Bridge Filter to be Off br Default*/
	br->br_filter_active = 0;

	/*TXTL-IGMP01 25-Nov-2004: Set default bridge multicast mode */
#ifdef CONFIG_BRIDGED_MULTICAST
	br_mfd_init(br);
#endif
...
int br_del_bridge(char *name)
{
	...
	/* TXTL-IGMP01 02 Dec 2004 : delete all mfd specific timers running*/
#ifdef CONFIG_BRIDGED_MULTICAST
	br_mfd_close(br);
#endif
