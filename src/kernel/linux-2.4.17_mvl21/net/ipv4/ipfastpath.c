...
int ip_forward(struct sk_buff *skb)
{
    ...
    if ((retVal == 0) && (iph->ttl > 1))
    {
        ...
        /* Jyothi - This is work around to prevent the TOS bits from overwriting 
         * skb->priority(setto 0xFxxx) that has been determined from the VLAN p_bits */

        if (!(skb->priority & 0xF000))
