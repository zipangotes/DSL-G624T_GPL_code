 ...
 *
 *	Version: $Id: ipmr.c,v 1.4 2004/12/27 11:33:11 gopal_kr Exp $
 *
 *	Fixes:
...
#if 1
#define IPMR_DEBUGP(format, args...)
#else
#define IPMR_DEBUGP printk
#endif
...
{
	struct sk_buff *skb;
#ifdef CONFIG_MULTIPLE_UPSTREAMS
...
#ifdef CONFIG_MULTIPLE_UPSTREAMS
	while(c->mfc_plist != NULL)
	{
		mfc_pnode = c->mfc_plist;
		c->mfc_plist = c->mfc_plist->next;
                kfree(mfc_pnode);
	}
#endif
...
/**************************************************************************
 * FUNCTION NAME : ipmr_cache_find
 **************************************************************************
 * DESCRIPTION   :
 * 	The function finds a matching MR CACHE entry for a packet with a 
 * 	specified multicast group address. The previous implementation was 
 * 	changed to no longer match the source of the packet. 
 * 	
 * RETURNS		 :
 * 	 NULL		- No match.
 * 	 Non NULL 	- Succesfull Match.
 ***************************************************************************/
#if 0
...
{	
	...
	/* Create a hash line for the multicast group. The source IP address
	 * is a DONT Care i.e. 0.0.0.0 */
...
ipmr_cache_unresolved(vifi_t vifi, struct sk_buff *skb)
{
	...
	if (c == NULL) {
		 ...
		 */
#ifdef CONFIG_MULTIPLE_UPSTREAMS
		mfc_pnode = (struct mfc_parent_list *)kmalloc(sizeof(struct mfc_parent_list), GFP_ATOMIC);
		if(mfc_pnode == NULL)
		{
                     	kmem_cache_free(mrt_cachep, c);
			printk(KERN_ERR "Allocation Failure: ipmr_cache_unresolved\n");
			kfree_skb(skb);
			return -ENOBUFS;
		}
		mfc_pnode->mfcc_parent = -1;
		mfc_pnode->next = c->mfc_plist;
		c->mfc_plist = mfc_pnode;
#else
		c->mfc_parent=-1;
#endif
...
int ipmr_mfc_add(struct mfcctl *mfc, int mrtsock)
{
...
#ifdef	CONFIG_MULTIPLE_UPSTREAMS
	struct mfc_parent_list* pnode = NULL ,*mfc_pnode = NULL;
#endif
	...
	if (c != NULL) {
		write_lock_bh(&mrt_lock);
#ifdef CONFIG_MULTIPLE_UPSTREAMS
		for(pnode = mfc->mfcc_plist,mfc_pnode = c->mfc_plist; (pnode != NULL) && (mfc_pnode != NULL); pnode = pnode->next, mfc_pnode = mfc_pnode->next)
		{ 
			mfc_pnode->mfcc_parent = pnode->mfcc_parent; 
		}
#else
...
#ifdef CONFIG_MULTIPLE_UPSTREAMS
	for(pnode = mfc->mfcc_plist; pnode != NULL; pnode = pnode->next)
	{ 
		mfc_pnode = (struct mfc_parent_list *)kmalloc(sizeof(struct mfc_parent_list), GFP_ATOMIC);
		if(mfc_pnode == NULL)
		{
			...
			printk(KERN_ERR "Allocation Failure: ipmr_mfc_add\n");
			return -ENOMEM;
		}	
		mfc_pnode->mfcc_parent = pnode->mfcc_parent; 
		mfc_pnode->next = c->mfc_plist;
...
#ifdef CONFIG_MULTIPLE_UPSTREAMS
		while(uc->mfc_plist != NULL)
		{
			mfc_pnode = uc->mfc_plist;
			uc->mfc_plist = uc->mfc_plist->next;
               	        kfree(mfc_pnode);
		}
#endif
...
int ip_mroute_setsockopt(struct sock *sk,int optname,char *optval,int optlen)
{
	...
	switch(optname)
	{
		...
		case MRT_DEL_VIF:
            ...
            /* PANKAJ: This check has been committed out because after creating multiple
             * connections it could be possible that the network_index which is passed
             * to this ioctl through the "vif.vifc_vifi" exceeds the MAXVIFS i.e. 32.
             * Once VIF creation fails the IGMP application stops working. */
#if 0
...
static inline int ipmr_forward_finish(struct sk_buff *skb)
{
	...
	IPMR_DEBUGP(KERN_INFO "mr_forw_fin : outgoing interface : %s\n", dev->name);
			   ...
			   int vifi, int last)
{
	...
	if (vif->flags&VIFF_TUNNEL) {
		...
		if (ip_route_output(&rt, iph->daddr, 0, RT_TOS(iph->tos), vif->link)) {
			IPMR_DEBUGP(KERN_INFO "ipmr_queue_xmit : ip_route_output failed for %04x\n", iph->daddr);
			return;
		}
	}
...
{
	int psend = -1;
	int vif, ct, vif_found = 0;
#ifdef CONFIG_MULTIPLE_UPSTREAMS
	struct mfc_parent_list *mfc_pnode = cache->mfc_plist;
#endif
...
#ifdef CONFIG_MULTIPLE_UPSTREAMS
	while(mfc_pnode != NULL)
	{
		vif = mfc_pnode->mfcc_parent;
		if (vif_table[vif].dev == skb->dev) {
			vif_found = 1;
			break;
		}
		mfc_pnode = mfc_pnode->next;
	}
#else
	vif = cache->mfc_parent;
#endif

	IPMR_DEBUGP(KERN_INFO "Multicast Pkt rxed on %s ; Cache entry parent is %d\n", skb->dev->name, vif);
	
	/*
	 * Wrong interface: drop packet and (maybe) send PIM assert.
	 */
#ifndef CONFIG_MULTIPLE_UPSTREAMS
	if (vif_table[vif].dev != skb->dev) {
#else
	if (vif_found == 0) {
#endif
		...
		IPMR_DEBUGP(KERN_INFO "mr_forward : No forward from %s\n", skb->dev->name);
		goto dont_forward;
	}
	...
	for (ct = cache->mfc_un.res.maxvif-1; ct >= cache->mfc_un.res.minvif; ct--) {
		if (skb->nh.iph->ttl > cache->mfc_un.res.ttls[ct]) {
			if (psend != -1) {
				IPMR_DEBUGP(KERN_INFO "ip_mr_forward : sending on vif : %d\n", psend);
		...
		{
			IPMR_DEBUGP(KERN_INFO "skb->nh.iph->ttl :%x\n",skb->nh.iph->ttl);
			IPMR_DEBUGP(KERN_INFO "mfc ttl:%x\n",cache->mfc_un.res.ttls[ct]);
		}
	}
	if (psend != -1) {
		IPMR_DEBUGP(KERN_INFO "ip_mr_forward : sending on vif : %d\n", psend);
...
int ip_mr_input(struct sk_buff *skb)
{
	...
	IPMR_DEBUGP(KERN_INFO "\nip_mr_forward : Cache entry found for %04x, pkt from %s\n", skb->nh.iph->daddr, skb->dev->name);
...
#ifdef CONFIG_MULTIPLE_UPSTREAMS
	struct mfc_parent_list * mfc_pnode = c->mfc_plist;
	while(mfc_pnode != NULL)
	{
	dev = vif_table[mfc_pnode->mfcc_parent].dev;
#else
	dev = vif_table[c->mfc_parent].dev;
#endif
	if (dev)
...
static int ipmr_mfc_info(char *buffer, char **start, off_t offset, int length)
{
	...
	for (ct=0;ct<MFC_LINES;ct++) 
	{
		for(mfc=mfc_cache_array[ct]; mfc; mfc=mfc->next)
		{
			 ...
			 */
#ifdef CONFIG_MULTIPLE_UPSTREAMS
			size = sprintf(buffer+len, "%08lX %08lX",(unsigned long)mfc->mfc_mcastgrp, (unsigned long)mfc->mfc_origin);
			mfc_pnode = mfc->mfc_plist;
			first_line = 1;
			while(mfc_pnode != NULL)
			{
				if(first_line == 1)
				{
					size += sprintf(buffer+len+size,"%3d", mfc_pnode->mfcc_parent);
					size += sprintf(buffer+len+size," %8ld %8ld %8ld",
						mfc->mfc_un.res.pkt,
