 ...
 *
 * Version:	$Id: ip_input.c,v 1.1.1.1 2002/03/18 15:12:19 jharrell Exp $
 *
...
int ip_local_deliver(struct sk_buff *skb)
{
    ...
    if (opt == NULL)
    {
        /* There were no IP options. Get the IP Header and compute the cheksum. */
        struct iphdr*   iph = skb->nh.iph;
    	if (ip_fast_csum((u8 *)iph, iph->ihl) != 0)
        {
            /* Checksum error. Increment statistics and drop the packet. */
    ...
    {
        /* IP Options were present in the packet. This means that the checksum has already
         * been computed in the "ip_process_option". Continue*/
    }
#endif /* TI_SLOW_PATH */
...
int ip_process_option (struct sk_buff *skb, struct net_device *dev)
{
    ...
    /* PANKAJ: If there is an option; this is slow path code. We need to compute the
     * checksum here irrespective of where the packet is destined because once we 
     * process and compile the option it might change the packet itself and so for
     * "locally" destined packets the checksum calculation done in "ip_local_deliver"
     * will fail. This is called from before the routing phase so we dont know where
