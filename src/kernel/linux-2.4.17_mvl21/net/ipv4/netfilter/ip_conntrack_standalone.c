...
print_expect(char *buffer, const struct ip_conntrack_expect *expect)
{
    ...
    /* PANKAJ: Debugging the crash when there is no valid helper matching the expectation. Occurs
     * with access control... */
    if ((expect == NULL) || (expect->expectant == NULL) || (expect->expectant->helper == NULL))
    {
        len = sprintf(buffer, "No helper registered.\n");
        return len;
    }
...
print_conntrack(char *buffer, const struct ip_conntrack *conntrack)
{
	...
	len = sprintf(buffer, "0x%p %-8s %u %lu ",
		      conntrack, proto->name,
...
list_conntracks(char *buffer, char **start, off_t offset, int length)
{
    ...
    printk ("DEBUG: IP Conntrack Count %d.\n", atomic_read(&ip_conntrack_count));
			       ...
			       int (*okfn)(struct sk_buff *))
{
    ...
    if (hooknum == NF_IP_LOCAL_IN)
        printk ("DEBUG: LOCAL_IN skb 0x%p \n", *pskb);
    else
        printk ("DEBUG: POST-ROUTING skb 0x%p conntrack 0x%p Count=%d.\n", *pskb, (*pskb)->nfct->master, atomic_read(&ip_conntrack_count));
#endif

	/* We've seen it coming out the other side: confirm it */
	retVal = ip_conntrack_confirm(*pskb);

    /* FIX: The LAST Stage the packet has passed through all the hooks. From this point on
     * it can never ever be pushed back through connection tracking. */
#if 1
...
{
#if 0
    printk ("DEBUG: LOCAL_OUT skb 0x%p conntrack Count=%d.\n", *pskb, atomic_read(&ip_conntrack_count));
#endif
