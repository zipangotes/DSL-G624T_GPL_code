...
#if 0
    printk ("DEBUG: Destroying conntrack 0x%p Count=%d Use=%d.\n", ct, atomic_read(&ip_conntrack_count), atomic_read(&nfct->use));
#endif 
...
#if 1
    {
        struct nf_conntrack* nfct = (struct nf_conntrack *)&ct->ct_general;
//        printk ("DEBUG: Death by Timeout 0x%p Count=%d Use=%d.\n", ct, atomic_read(&ip_conntrack_count), atomic_read(&nfct->use));
    	if (atomic_dec_and_test(&nfct->use))
	    	destroy_conntrack(nfct);
    }
#endif
}
...
/* Find a connection corresponding to a tuple. */
struct ip_conntrack_tuple_hash *
ti_nsp_ip_conntrack_find_get(const struct ip_conntrack_tuple *tuple,
		      const struct ip_conntrack *ignored_conntrack)
{
	struct ip_conntrack_tuple_hash *h;
	...
	*ctinfo = IP_CT_RELATED;
	...
	if (!h) {
		...
		if (hooknum == NF_IP_LOCAL_OUT)
        {
            ...
            return (struct ip_conntrack *)0xFFFFFFFF;
        }
			     ...
			     int (*okfn)(struct sk_buff *))
{
	...
	int set_reply;
	int ret;
    	register struct sk_buff*  ptr_skb = (*pskb);
    	register struct iphdr* ptr_iphdr;
...
/* Doesn't cover locally-generated broadcast, so not worth it. */
	...
	proto = ip_ct_find_proto((*pskb)->nh.iph->protocol);
        ...
        ptr_iphdr = (struct iphdr*)((*pskb)->nh.iph);
    	//ptr_iphdr = (struct iphdr*)ptr_skb->nh.iph;

	/* If Expected to drop unknown protocol packets drop it here */
 	if(gDropUnknownProto && ( proto->proto == 0 ) && (ptr_iphdr->protocol != 2)
           && (ptr_iphdr->protocol != IPPROTO_GRE) ) // IGMP = 2
   		return NF_DROP;
    ...
    if (hooknum == NF_IP_PRE_ROUTING)
        printk ("DEBUG: PRE-ROUTING skb 0x%p conntrack 0x%p Count=%d.\n", *pskb, ct, atomic_read(&ip_conntrack_count));
    else
        printk ("DEBUG: LOCAL_OUT   skb 0x%p conntrack 0x%p Count=%d.\n", *pskb, ct, atomic_read(&ip_conntrack_count));
#endif
...
ip_ct_gather_frags(struct sk_buff *skb)
{
	...
	if (sk) {
	 ...
	 /* BUG#: It was observed that in certain test cases, i.e. multicast fragmented
         * packets, we would freeze the box trying to compute the checksum. The IP
         * packet, when this happened, seemed to have incorrect header information. So
         * we would run forever in the checksum compuation function. A check was
         * added to verify that only valid IP packets were processed further. */
