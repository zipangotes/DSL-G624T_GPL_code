...
{
	return sprintf(buffer, "SPI=%x ",ntohl(tuple->dst.u.esp.spi));
}
                                       ...
                                       const struct ip_conntrack *ct)
{
       return sprintf(buffer, "OUT SPI=%x ,IN SPI = %x \n",
	   	               ntohl(ct->tuplehash[0].tuple.dst.u.esp.spi),ntohl(ct->tuplehash[1].tuple.dst.u.esp.spi));
}
      ...
      switch(state)
      {
        ...
        case IN:
          ...
          if((ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.esp.spi == out_tuple.dst.u.esp.spi) &&
             (ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.ip == out_tuple.src.ip))
          {
