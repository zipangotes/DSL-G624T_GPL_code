 ...
 *
 *	Version: $Id: igmp.c,v 1.3 2004/12/27 11:33:11 gopal_kr Exp $
 *
 *	Authors:
...
#if 1
#define IP_IGMP_DEBUGP(format, args...)
#else
#define IP_IGMP_DEBUGP printk
#endif
...
static int igmp_send_report(struct net_device *dev, u32 group, int type)
{
	...
	IP_IGMP_DEBUGP(KERN_INFO "Sending %d for group %4x on device %s\n", type, group, dev->name);
...
static void igmp_timer_expire(unsigned long data)
{
	...
	{	
		IP_IGMP_DEBUGP(KERN_INFO "Timer Expired: Sending membership report on %s for group %x\n",in_dev->dev->name, im->multiaddr);
...
static void igmp_heard_report(struct in_device *in_dev, u32 group)
{
	...
	IP_IGMP_DEBUGP(KERN_INFO "IGMP Heard Report on %s for group %4x\n", in_dev->dev->name, group);
	...
	if (in_dev->mc_list == NULL) {
		IP_IGMP_DEBUGP(KERN_INFO "IGMP Heard Report : mc_list for %s is empty\n", in_dev->dev->name);
	}
			     ...
			     u32 group)
{
	...
	IP_IGMP_DEBUGP(KERN_INFO "IGMP Heard Query on %s for group %4x with max resp time %d\n", in_dev->dev->name, group, max_resp_time);
	...
	if (in_dev->mc_list == NULL) {
		IP_IGMP_DEBUGP(KERN_INFO "IGMP Heard Query : mc_list for %s is empty\n", in_dev->dev->name);
	}
	...
	for (im=in_dev->mc_list; im!=NULL; im=im->next) {
		...
		if (im->multiaddr == IGMP_ALL_HOSTS)
			continue;
		IP_IGMP_DEBUGP(KERN_INFO "IGMP Heard Query : timer started with max delay %d\n", max_delay);
...
static void igmp_group_added(struct ip_mc_list *im)
{
	...
	if (im->multiaddr == IGMP_ALL_HOSTS)
		return;
	IP_IGMP_DEBUGP(KERN_INFO "IGMP group membership added on %s for group %x\n",im->interface->dev->name, im->multiaddr);
	spin_lock_bh(&im->lock);
