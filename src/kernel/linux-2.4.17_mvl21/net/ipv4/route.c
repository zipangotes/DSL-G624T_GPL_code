 ...
 *
 * Version:	$Id: route.c,v 1.2 2004/11/25 06:44:04 gopal_kr Exp $
 *
...
#if 1
#define IP_IPMR_DEBUGP(format, args...)
#else
#define IP_IPMR_DEBUGP printk
#endif
...
/* NSP Optimizations: The structure was added to the header file and the
 * variables 'rt_hash_table' and 'rt_hash_mask' were changed from static
 * so that they could be accessed in the fast path. */
#ifdef TI_SLOW_PATH
...
#else
/* NSP Optimization: These variables are not static and are used in the FAST
 * Path. */
struct rt_hash_bucket 	*rt_hash_table;
...
static void rt_set_nexthop(struct rtable *rt, struct fib_result *res, u32 itag)
{
	...
	if (fi) {
		...
		if (fi->fib_mtu == 0) {
			...
			rt->u.dst.pmtu = (rt->u.dst.dev->unused_alignment_fixer == 0) ? rt->u.dst.dev->mtu : 
								min(rt->u.dst.dev->mtu, rt->u.dst.dev->unused_alignment_fixer);
#else
				...
				u8 tos, struct net_device *dev, int our)
{
	...
	IP_IPMR_DEBUGP(KERN_INFO "input_mc : Rxed multicast pkt on %s interface\n", dev->name);
	...
	if (our) {
		IP_IPMR_DEBUGP(KERN_INFO "input_mc : local_deliver multicast pkt rxed from %s interface\n", dev->name);
	...
	if (!LOCAL_MCAST(daddr) && IN_DEV_MFORWARD(in_dev)) {
		IP_IPMR_DEBUGP(KERN_INFO "input_mc : ip_mr_input handle multicast pkt rxed from %s interface\n", dev->name);
			...
			u8 tos, struct net_device *dev)
{
	...
	if (skb->protocol != __constant_htons(ETH_P_IP)) {
        ...
        out_dev->dev->if_port = dev->if_port;
#endif
	rth = dst_alloc(&ipv4_dst_ops);
	if (!rth)
		goto e_nobufs;
...
/* This function has been retained because it is called from other modules
 * like ARP also. This function is SLOW PATH. */
...
#ifndef TI_SLOW_PATH
/**************************************************************************
 * FUNCTION NAME : ip_route_exception
 **************************************************************************
 * DESCRIPTION   :
 *  The function is called from the FAST PATH when a routing exception 
 *  takes place, which means that the routing entry was not in the cache or
 *  destination address was a MULTICAST Address. The function hence is 
 *  a slow path.
 *
 *  RETURNS      :
 *      0               - Success.
 ...
 *
 *  NOTES        : 
 *      The function cleans the skb on error. This removes the kfree_skb 
 *      code from the FAST PATH.
 **************************************************************************/
int ip_route_exception(struct sk_buff *skb, u32 daddr, u32 saddr, u8 tos, struct net_device *dev)
{
    int retVal;
	...
	if (MULTICAST(daddr)) 
    {
		...
		if ((in_dev = __in_dev_get(dev)) != NULL) 
        {
			    ...
			    ) 
            {
                ...
                /* Check if there was an error. The functions return 0 on
                 * SUCCESS anything else is an ERROR. */
                retVal = ip_route_input_mc(skb, daddr, saddr, tos, dev, our);
				if (retVal != 0)
                {
                    /* YES. Clean memory. */
                    kfree_skb(skb);
        ...
        /* Clean the memory on error. */
        kfree_skb(skb);
		read_unlock(&inetdev_lock);
		return -EINVAL;
	}

    /* Check if there was an error. The functions return 0 on SUCCESS anything
     * else is an error. */
    retVal = ip_route_input_slow(skb, daddr, saddr, tos, dev);
	if (retVal != 0)
    {
        /* YES. Clean memory. */
        kfree_skb(skb);
