 ...
 *		The IP to API glue.
 *		
 * Version:	$Id: ip_sockglue.c,v 1.2 2003/12/04 18:28:11 mhassler Exp $
 *
 * Authors:	see ip.c
 *
 * Fixes:
...
int ip_setsockopt(struct sock *sk, int level, int optname, char *optval, int optlen)
{
	...
	switch (optname) {
        ...
        {
            extern int voice_register_protocol_port (unsigned int port);
            ...
            if (optlen >= sizeof(int)) 
            {
                if (val & 0x8000)
                {
                    extension_mode = 1;
                    if (val & 0x4000)
                        sk->priority = 0xFFFF; /* EF1 -- No mapping */
                    else
                        sk->priority = 6;
                    printk("DEBUG: Extension Mode SKB->Priority=0x%x\n",sk->priority);

                    /* Reset the extension bits. */
                    val = val & 0xFF;
                }
            }

            /* If non extension mode or if the MSB of 2nd byte was not set.*/
    		if (sk->type == SOCK_STREAM) {
	    			val &= ~3;
	        ...
	        if (sk->protinfo.af_inet.tos != val) 
            {
                ...
                /* Only in non-extension mode set the priority. In extension mode the priority
                 * has already been set above. */
                if (extension_mode == 0)
    				sk->priority = rt_tos2priority(val);
	    		sk_dst_reset(sk);
            }
            printk("DEBUG: SKB->Priority=0x%x\n",sk->priority);

            /* Catch all voice socket bind and remember the protocol ports used by the voice. */
            if (sk->sport != 0x0)
            {
                voice_register_protocol_port (sk->sport);
            }
            else
            {
                printk ("Error: Voice/Kernel mismatch. Verify bind before setsockopt.\n");
            }
			break;
        }
		case IP_TTL:
