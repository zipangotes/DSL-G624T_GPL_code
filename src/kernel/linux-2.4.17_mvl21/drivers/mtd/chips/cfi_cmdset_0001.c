 ...
 *
 * $Id: cfi_cmdset_0001.c,v 1.8 2002/10/10 18:06:03 jyothi Exp $
 *
 *
...
/* IMPORTANT: Define this flag to make write blocking, i.e the delay in
 * do_write_onword will not relinquish time slice for the read (do_read_oneword)
 * and erase (do_erase_oneblock) routines.*/
#define BLOCKING_WRITE
...
#ifdef BLOCKING_WRITE
	udelay(chip->word_write_time);
#else
	spin_unlock_bh(chip->mutex);
	cfi_udelay(chip->word_write_time);
	spin_lock_bh(chip->mutex);
#endif /* BLOCKING_WRITE */
...
typedef int (*varsize_frob_t)(struct map_info *map, struct flchip *chip, unsigned long adr);
static int cfi_intelext_varsize_frob(struct mtd_info *mtd, varsize_frob_t frob, loff_t ofs, size_t len)
{
        struct map_info *map = mtd->priv;
        struct cfi_private *cfi = map->fldrv_priv;
        unsigned long adr;
...
int cfi_intelext_erase_varsize(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	ret = cfi_intelext_varsize_frob(mtd, do_erase_oneblock, ofs, len);
        if (ret)
       		return ret;
...
static int do_printlockstatus_oneblock(struct map_info *map, struct flchip *chip, unsigned long adr)
{
	struct cfi_private *cfi = map->fldrv_priv;
	int ofs_factor = cfi->interleave * cfi->device_type;
...
static int do_lock_oneblock(struct map_info *map, struct flchip *chip, unsigned long adr)
{
	struct cfi_private *cfi = map->fldrv_priv;
	__u32 status, status_OK;
	unsigned long timeo;
...
static int cfi_intelext_lock(struct mtd_info *mtd, loff_t ofs, size_t len)
{
	int ret;
        ...
        cfi_intelext_varsize_frob(mtd, do_printlockstatus_oneblock, ofs, len);
#endif

       	ret = cfi_intelext_varsize_frob(mtd, do_lock_oneblock, ofs, len);
#ifdef DEBUG_LOCK_BITS
        printk(KERN_DEBUG __FUNCTION__ ": after locking, ret=%d\n", ret);
        cfi_intelext_varsize_frob(mtd, do_printlockstatus_oneblock, ofs, len);
#endif
...
static int do_unlock_oneblock(struct map_info *map, struct flchip *chip, unsigned long adr)
{
	struct cfi_private *cfi = map->fldrv_priv;
	__u32 status, status_OK;
	unsigned long timeo;
...
static int cfi_intelext_unlock(struct mtd_info *mtd, loff_t ofs, size_t len)
{
	...
	cfi_intelext_varsize_frob(mtd, do_printlockstatus_oneblock, ofs, len);
 	...
 	ret = cfi_intelext_varsize_frob(mtd, do_unlock_oneblock, ofs, len);
