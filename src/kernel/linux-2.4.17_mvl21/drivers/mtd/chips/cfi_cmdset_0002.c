 ...
 * This code is GPL
 *
 * $Id: cfi_cmdset_0002.c,v 1.2 2002/09/27 14:15:06 sneha Exp $
 *
 */
...
static int do_write_oneword(struct map_info *map, struct flchip *chip, unsigned long adr, __u32 datum, int fast)
{
 ...
 retry:
	...
	if (fast) { /* Unlock bypass */
		if (CFIDEV_BUSWIDTH == 8)
			cfi_send_word_cmd(0xA0, 0, base, map, cfi, cfi->device_type, NULL);
		else
		...
		if (CFIDEV_BUSWIDTH == 8) {
			cfi_send_word_cmd(0xAA, cfi->addr_unlock1, base, map, cfi, CFI_DEVICETYPE_X8, NULL);
			cfi_send_word_cmd(0x55, cfi->addr_unlock2, base, map, cfi, CFI_DEVICETYPE_X8, NULL);
			cfi_send_word_cmd(0xA0, cfi->addr_unlock1, base, map, cfi, CFI_DEVICETYPE_X8, NULL);
		}
		else {
...
static int cfi_amdstd_erase_varsize(struct mtd_info *mtd, struct erase_info *instr)
{
  ...
  /* Hai: Patch for Am29LV640B flash */
  if ( instr->addr == 0 ) 
  {
     if( instr->len == 0x2000 )
       instr->len = 0x10000;
  }
  else if( instr->addr < 0x10000 )
     goto end;
