...
#define PHY_SMODE_SIZE      (6)        /*  28 Bits used */
...
#define PHY_CHNG_SIZE      (1)         /*  29 Bits used */
...
int cpswHalCommonMiiMdioInit(PHY_DEVICE *PhyDev, bit32u miibase, bit32u inst, bit32u PhyMask, bit32u MLinkMask, bit32u MdixMask, bit32u ResetReg, bit32u ResetBit, bit32u MdioBusFreq, bit32u MdioClockFreq,int verbose,int (*osprintf)(const char *Format, ...))
  {
...
void cpswHalCommonMiiMdioSetPhyMode(PHY_DEVICE *PhyDev,bit32u PhyMode)
  ...
  *PhyState&=~PHY_SMODE_MASK;
  ...
  if ((CurrentState==NWAY_START)||
   ...
   mdioPrintf("SetPhyMode:%08X Auto:%d, FD10:%d, HD10:%d, FD100:%d, HD100:%d LPBK:%d\n", PhyMode,
...
int cpswHalCommonMiiMdioTic(PHY_DEVICE *PhyDev)
  ...
  switch(CurrentState&PHY_STATE_MASK)
    {
    case INIT:       _cpswHalCommonMiiMdioInitState(PhyDev);      break;
    case FINDING:    _cpswHalCommonMiiMdioFindingState(PhyDev);   break;
    case FOUND:      _cpswHalCommonMiiMdioFoundState(PhyDev);     break;
    case NWAY_START: _cpswHalCommonMiiMdioNwayStartState(PhyDev); break;
    case NWAY_WAIT:  _cpswHalCommonMiiMdioNwayWaitState(PhyDev);  break;
    case LINK_WAIT:  _cpswHalCommonMiiMdioLinkWaitState(PhyDev);  break;
    case LINKED:     _cpswHalCommonMiiMdioLinkedState(PhyDev);    break;
    case LOOPBACK:   _cpswHalCommonMiiMdioLoopbackState(PhyDev);  break;
    default:         _cpswHalCommonMiiMdioDefaultState(PhyDev);   break;
    }
