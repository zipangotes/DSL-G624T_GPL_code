			      ...
			      size_t count)
{
    ...
    /* This is the control path and should not be classified by the QOS U32 classifiers.
     * They work on the network header. So lets initialize it to NULL, since alloc_skb does
...
ppp_send_frame(struct ppp *ppp, struct sk_buff *skb)
{
  ...
  int skip = 0;
    ...
    pport = (u32 *)(skb->data + 2 + (skb->data[2] & 0x0F)*4);
    if(( (((*pport)& 0x000000FF) << 8) |
         (((*pport) & 0x0000FF00) >> 8) ) != 520)
	    ppp->last_xmit = jiffies;
    else
	    skip = 1;
#endif /* CONFIG_PPP_FILTER */
	 ...
	 */
	if (ppp->flags & SC_LOOP_TRAFFIC && ( skip != 1)) {
  ...
  u32 *pport = NULL;
	...
	if (npi < 0) {
		...
		*skb_push(skb, 2) = 0;
		...
		skb_pull(skb, 2);
#else
		/* The IP header start at a 2 byte offset. We extract the
       destination port field from the TCP/UDP packet
       and look for port 520 used by RIP which is then not
       treated as an activity packet */
    pport = (u32 *)(skb->data + 2 + (skb->data[2] & 0x0F)*4);
    if(( (((*pport)& 0x000000FF) << 8) |
         (((*pport) & 0x0000FF00) >> 8) ) != 520)
      ppp->last_recv = jiffies;
		...
		if ((ppp->dev->flags & IFF_UP) == 0
			...
			if (clones == 0) {
		              printk(KERN_ERR "PPP rcv frame : no mem for cloned skb\n");
			      goto err;
		        }
		        else {
