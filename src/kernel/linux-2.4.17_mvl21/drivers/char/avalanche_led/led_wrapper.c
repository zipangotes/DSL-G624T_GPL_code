...
static void *avalanche_led_register_dsl(const char *module_name, int instance_num);                 
static int avalanche_led_action_dsl(void *handle,int state_id);                                     
int avalanche_led_config_get (LED_CONFIG_T *led_cfg,int module_id,int instance, int state);

led_reg_t adsl_led_objs[4];
STATE_ENTRY_T adsl_states[MAX_STATE_ENTRIES];
MODULE_INSTANCE_T *dsl_mod = NULL; 
MODULE_INSTANCE_T *new_dsl_mod = NULL; 
...
static void free_state(STATE_ENTRY_T *state)
{
    ...
    while(curr != state)
    {
        ...
        PAL_osMemFree(0,curr,sizeof(curr));
        if(prev)
        {   
            prev->next = NULL;
        }
...
static MODULE_INSTANCE_T* get_module(char * name,int instance)
{
    ...
    if(module_id == MAX_MODULE_ENTRIES)
    {
	    ...
	    {
		PAL_osMemFree(0,modules[module_id],sizeof(MODULE_ENTRY_T));
...
int avalanche_led_set_config(LED_CONFIG_T *led_cfg)
{
	...
	/* For now support configurations for LEDs connected to
         * one GPIO pin.  */
	led_cfg->gpio_num = 1;
        ...
        if(led_cfg->state < MAX_STATE_ENTRIES)
        {
            ...
            if(!(module->states[state]))
            {
                ...
                module->states[state] = &adsl_states[state];
            }
	...
	/* Multi pin LED configuration not supported */
#if 0               
...
void *avalanche_led_register_dsl(const char * mod_name,int instance)
{
        void *p;
...
int avalanche_led_action_dsl(void *module, int state_id)
{
...
void deregister_led_drv( int led_num)
{
    ...
    if(led_index >=0 && led_index <= 2)
    {  
        printk("deregister_led_drv led index = %d \n",led_index);
...
void led_operation(int mod,int state_id)
{
    ...
    {
    STATE_ENTRY_T *state =((MODULE_INSTANCE_T *)(dsl_mod))->states[state_id];
    int gpio = state->gpio->gpio_id;
    if((gpio>31) && (gpio<37))
	    avalanche_led_action_dsl(dsl_mod,state_id); 
    else
    	avalanche_led_action(new_dsl_mod,state_id); 
	}
}
...
static int __init  led_init(void)
{
   ...
   dsl_mod = avalanche_led_register_dsl("adsl",0);
    new_dsl_mod = avalanche_led_register("adsl",0);
    return 0;
