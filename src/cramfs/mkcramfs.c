			...
			/* block pointers & data expansion allowance + data */
                        if(entry->size) 
                                *fslen_ub += ((4+26)*((entry->size - 1) / blksize + 1)
                                              + MIN((int)(entry->size + 3), st.st_blocks << 9));
                        else 
                                *fslen_ub += MIN((int)(entry->size + 3), st.st_blocks << 9);
                }
...
int main(int argc, char **argv)
{
	...
	if (offset != (unsigned int)written) {
