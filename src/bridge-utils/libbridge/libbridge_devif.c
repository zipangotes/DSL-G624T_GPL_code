...
void __copy_bf_entry(struct filter_entry *ent, struct __br_filter_entry *bf)
{
  memcpy(ent->sport, bf->sport, sizeof(bf->sport));
  memcpy(ent->dport, bf->dport, sizeof(bf->dport));
  memcpy(ent->src_mac_addr, bf->src_mac_addr, 6);
  memcpy(ent->dst_mac_addr, bf->dst_mac_addr, 6);
  ent->proto = bf->proto;
  ent->access_type = bf->access_type;
  ent->frames_matched = bf->frames_matched;

}

int br_show_filter(struct bridge *br, struct filter_entry *entry, int num)
{
  struct __br_filter_entry bf[num];
  int numread, i;

  if ((numread = br_device_ioctl(br, BRCTL_SHOW_FILTER_ENTERIES, 
				(unsigned long)bf, num, 0)) < 0)
    return errno;

  for (i = 0; i < numread; i++)
    __copy_bf_entry(entry+i, bf+i);

  return numread;
}

int br_set_filter_state(struct bridge *br, int filter_active)
{
  if (br_device_ioctl(br, BRCTL_SET_FILTER_STATE, filter_active,
		      0, 0) < 0)
    return errno;
...
int br_set_filter_entry(struct bridge *br, struct filter_entry *br_filter)
{
  struct __br_filter_entry *filter;

  filter = (struct __br_filter_entry *)br_filter;

  if (br_device_ioctl(br, BRCTL_SET_FILTER_ENTRY, (unsigned long)filter,
		      0, 0) < 0)
    return errno;
...
int br_del_filter_entry(struct bridge *br, struct filter_entry *br_filter)
{
  struct __br_filter_entry *filter;

  filter = (struct __br_filter_entry *)br_filter;

  if (br_device_ioctl(br, BRCTL_DELETE_FILTER_ENTRY, (unsigned long)filter,
		      0, 0) < 0)
    return errno;
...
int br_flush_filter(struct bridge *br)
{
	if (br_device_ioctl(br, BRCTL_FLUSH_FILTER_ENTERIES, 0, 0, 0) < 0)
	  return errno;
...
/* TXTL-IGMP01 - Added for MFD Entries at Bridge */
void __copy_mfd(struct mfd_entry *ent, struct __mfd_entry *f)
{	
	int i=0;

	ent->grp_addr = f->grp_addr;
	ent->no_ports = f->no_ports;
	for(i=0; i < ent->no_ports; i++)
	{
		ent->port_list[i] = f->port_list[i];
		ent->vinfo_list[i] = f->vinfo_list[i];
	}

}

int br_read_mfd(struct bridge *br, struct mfd_entry *mfds, int offset,int num)
{
	struct __mfd_entry mfd[num];
	int i;
	int numread;
	
	if ((numread = br_device_ioctl(br, BRCTL_MFD_DISPLAY,(unsigned long)mfd,0,0)) < 0)
		return errno;
 
	for( i=0;i<numread; i++)
		__copy_mfd(mfds+i, mfd+i);

	return numread;					

}

int br_read_mc_status(struct bridge *br, struct mc_status *mcs)
{
	struct __mc_status mcstat;
	
	if(br_device_ioctl(br, BRCTL_GET_MC_STATUS, (unsigned long)&mcstat,0,0) < 0)
		return errno;
	
	mcs->mc_forwarding = mcstat.mc_forwarding;
#ifdef CONFIG_BRIDGE_CONF_MODE
	mcs->conf_mode = mcstat.conf_mode;
	mcs->isQuerier = mcstat.isQuerier;
#endif
