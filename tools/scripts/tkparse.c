...
#define match_token(t, s) \
	...
	match_token( token_export, "export" );
	break;
...
static void do_source( const char * filename )
{
    ...
    int old_lineno;
    int offset;
    char kernelstring[200] = "../../src/kernel/linux-2.4.17_mvl21"; 
    char * buffer2;
    ...
    if ( infile == NULL )
    {
        //buffer2 = strcat(kernelstring , filename);
	/* replace the $TI_TARGET_KERNEL_PATH. variable with the kernel path */
	
	filename += 23;
	buffer2 = strcat(kernelstring, filename);	
 //       printf("debug:%s\n",buffer2);
	infile = fopen(buffer2, "r" );
    }
